  ...
  try {
    const result = await pool.query(
      `SELECT DISTINCT 
        payer_name
      FROM receipts
      WHERE payer_name IS NOT NULL AND TRIM(payer_name) != ''
      ORDER BY payer_name ASC`
    )

    console.log('📋 Found names:', result.rows.length)

    // ส่งแค่ชื่อสกุล ไม่มีคำนำหน้า
    const names = result.rows.map(row => row.payer_name)
